...
TEST_CASE("a legal move commits the new position exactly once") {
    GameState state(open_field());
    const MoveOutcome outcome = state.move(Direction::right);
...
TEST_CASE("a move into a wall is blocked and never commits") {
    ...
    for (const Direction direction :
        ...
        const MoveOutcome outcome = state.move(direction);
...
TEST_CASE("peek reports the outcome without mutating state") {
    ...
    const MoveOutcome moved = state.move(Direction::right);
...
TEST_CASE("movement round-trips across the interior") {
    GameState state(open_field());
    CHECK(state.move(Direction::right).result == MoveResult::moved);
    CHECK(state.move(Direction::down).result == MoveResult::moved);
    CHECK(state.actor_position() == Coordinates{1, 1});
    CHECK(state.move(Direction::left).result == MoveResult::moved);
    CHECK(state.move(Direction::up).result == MoveResult::moved);
    CHECK(state.actor_position() == Coordinates{0, 0});
}
